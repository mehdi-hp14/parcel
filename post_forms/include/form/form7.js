...
Vromansys.Item.TextList = function(el) {
	...
	function _renumberRows() {
		_table.find("tr").each(function(n, r) {
			...
			var continue_opration = r.find("input").get(0).name.lastIndexOf('attachments');
			if(continue_opration == -1)
			{
				var i = r.find("input").get(0);
				i.name = i.name.substring(0, i.name.lastIndexOf('-') + 1) + n;
				var i2 = r.find("input").get(1);
				i2.name = i2.name.substring(0, i2.name.lastIndexOf('-') + 1) + n;
			}
		});
	}
