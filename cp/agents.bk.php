					...
					foreach($_POST['carrier'] as $k => $v)
					{
						if($v!='' AND $v != null)
						{
							...
							else
							{
								...
								$tmpprice = $this->GetOfferPriceFromReceive($_POST['offer'][$k], $_POST['currecy'][$k], $currency, $extra, $percent);
								$tmpcost = $tmpprice;
								if($comision > 0)
								{
								...
								$tmp2 .= ($_c +1).') off=>'.$v.'=>'.number_format($tmpcost,2).'=>'.(($_POST['currecy'][$k]=='EUR' OR $_POST['currecy'][$k]=='USD') ? $currency : $_POST['currecy'][$k]).' | ';
								$_c++;
							}
						}
					}
...
cargo@europostexpress.co.uk<br>
								...
								$from = array('email'=>'quote@bookingparcel.com', 'name'=>'BookingParcel');
								...
								$mailer = new MailerClass();
								$mailer->SendMail($from, array('email'=>"cargo@europostexpress.co.uk", 'name'=>"EuroPost Express"), "The prices set by agent and Sent to user, ref EPX-".$this->country_iso[$this->data['from']]."-".$this->country_iso[$this->data['to']]."-".$this->data['id'], $message);
								$mailer = null;
								
								if($official_office!=null AND $official_office['master_email']!='' AND $official_office['master_email']!=null)
								{
									$mailer = new MailerClass();
									$mailer->SendMail($from, array('email'=>'quote@bookingparcel.com', 'name'=>'BookingParcel'), "The prices set by agent and Sent to user, ref EPX-".$this->country_iso[$this->data['from']]."-".$this->country_iso[$this->data['to']]."-".$this->data['id'], $message);
									$mailer = null;
...
cargo@europostexpress.co.uk<br>
								...
								$mails = explode("\n",$this->adata['emails']);
								$mailer = new MailerClass();
								$mailer->SendMail(array('email'=>array_pop($mails), 'name'=>$this->adata['cname']), array('email'=>"cargo@europostexpress.co.uk", 'name'=>"EuroPost Express"), "Quote Received for ref EPX-".$this->country_iso[$this->data['from']]."-".$this->country_iso[$this->data['to']]."-".$this->data['id'], $message, array(), $mails);
								$mailer = null;
