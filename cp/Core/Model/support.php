...
class SupportModel extends ModelBase
{
	public function GetTickets($uid, $page, $items_per_page)
	{
	   ...
	   // echo $page ."<br>";
		...
		$q = "SELECT `subject`,`id`, `content`, `c_date`, `status`, `usr_readed` FROM `tickets` WHERE `uid`=:uid AND `primary_p`=1 ORDER BY `c_date` DESC, `id` ASC LIMIT ".($page-1).",".$items_per_page."";
