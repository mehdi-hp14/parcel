...
$src_mbox = imap_open("{mail.europostexpress.com:143/novalidate-cert}" . $mailbox,"$src_username","$src_password") 
...
$dest_mbox = imap_open("{66.49.213.210:143/novalidate-cert}","$dest_username","$dest_password", OP_HALFOPEN)
	 or die("can't connect: ".imap_last_error()."\n");
...
if ($mailbox == "INBOX") {
  ...
  $dest_mailbox = "INBOX"; //getArchiveDateName($now);
...
$foo = $dest_mbox;
$dest_mbox = imap_reopen($dest_mbox, "{66.49.213.210:143/novalidate-cert}" . $dest_mailbox);
$dest_mbox = $foo;
...
for ($i=1; $i<=$da_no_msgs; $i++) {
  ...
  if (($archive_date == -1) || ($msg_date < $archive_date)) {
    ...
    if (imap_append($dest_mbox, "{66.49.213.210:143}".$dest_mailbox, $contents)) {
