...
function getScriptCall($folder_name, $folder_date_weeks, $delete_src_msg) {
  ...
  $script_call = "/home/europost/public_html/mig/scripts/archive.php $src_server $src_username \"$src_password\" $dest_server $dest_username \"$dest_password\" \"$folder_name\" $folder_date_weeks \"$delete_src_msg\"\n";
  return $script_call;
}
...
//$src_mbox = imap_open("{$src_server}:143","$src_username","$src_password")
//         or die("can't connect: ".imap_last_error());
$src_mbox = imap_open("{mail.europostexpress.com:143/novalidate-cert}","test@europostexpress.com","Fazel3771343")
         or die("can't connect: ".imap_last_error());
...
$list = imap_listmailbox($src_mbox,"{mail.europostexpress.com}","*");
if (is_array($list)) {
  reset($list);
} else {
...
while (list($key, $val) = each($list)) {
  ...
  $mailbox = ereg_replace("\}", "", $mailbox);
  $mailbox = ereg_replace("^$src_server", "", $mailbox);
